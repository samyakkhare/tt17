<?php
	...
	$sql = 'CREATE TABLE ' . $table . ' ( DC INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY, FN VARCHAR(20) NOT NULL, LN VARCHAR(20) NOT NULL,EMAIL VARCHAR(30) NOT NULL, REGNO VARCHAR(9) NOT NULL UNIQUE, PHNO VARCHAR(10) NOT NULL, CT VARCHAR(8) NOT NULL);';
	if($conn -> query($sql) === TRUE) {
	...
	$sql = 'CREATE TABLE ' . $table1 . ' ( UID VARCHAR(20) NOT NULL,logged INT,outtime TIMESTAMP(4),sysin TIMESTAMP(4));';
	if($conn -> query($sql) === TRUE) {
		echo 'Table ' . $table1 . ' created<br />';
	} else {
		echo 'Error creating table: ' . $conn -> error . '<br />';
	}
